...
### `faction-data-export.json` (8.6 KB)
...
- `cities` - Array of city objects:
  ...
  - `riverFleet` - Optional: Starting river fleet
  - `isCapital` - Optional: true if this is the capital
- `totalArmies`, `totalSeaFleets`, `totalRiverFleets` - Calculated totals
...
- Troll Country (Trolls) - Brown (#A86051)
...
These files are generated by the export functionality in `src/utils/exportGameData.ts`.

To regenerate:
```bash
npx tsx scripts/generateExportFiles.ts
```

Or use the in-game export buttons:
- Game Menu → "Export Map Data (JSON)"
- Game Menu → "Export Faction Data (JSON)"
...
**Separate Exports:**
- Map and faction data are always exported separately
- Allows independent use of map or faction data
- Easier to process and version control
...
Generated from Divine Right 3rd Edition rulebook with DR25 hex coordinate mappings.
